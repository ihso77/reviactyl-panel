...
import type { Server, NodeStats } from '@/lib/types';
...
export default function DashboardPage() {
  ...
  const avgCpu = onlineServers > 0
    ? Math.round(servers.filter((s) => s.status === 'online').reduce((sum, s) => sum + s.cpu, 0) / onlineServers)
    : 0;
        ...
        <Card className="border-border/50">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Progress value={totalMemoryLimit > 0 ? (totalMemory / totalMemoryLimit) * 100 : 0} className="h-2" />
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Servers</span>
                <span className="font-medium">{servers.length} total, {onlineServers} online</span>
              </div>
              <Progress value={servers.length > 0 ? (onlineServers / servers.length) * 100 : 0} className="h-2" />
            </div>
          </CardContent>
        </Card>
              ...
              <Link key={server.id} href={`/server/${server.id}`}>
                <Card className="border-border/50 transition-all duration-200 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 group cursor-pointer h-full">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">{server.description || 'No description'}</CardDescription>
                  </CardHeader>
                  <CardContent className="pt-0">
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <Progress value={server.memoryLimit > 0 ? (server.memory / server.memoryLimit) * 100 : 0} className="h-1.5 flex-1" />
                      ...
                      <div>
                        ...
                        <p className="text-xs font-medium truncate">{(server as any).node?.name || '—'}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
