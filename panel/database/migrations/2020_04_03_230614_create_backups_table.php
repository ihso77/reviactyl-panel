...
class CreateBackupsTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        $results = [];
        if ($driver === 'sqlite') {
            $tables = DB::select("SELECT name FROM sqlite_master WHERE type = 'table' AND name LIKE 'backup%' AND name NOT LIKE '%_plugin_bak'");
            foreach ($tables as $table) {
                $results[] = (object) ['TABLE_NAME' => $table->name];
            }
        } else {
